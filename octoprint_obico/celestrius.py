...
class Celestrius:
    ...
    def __init__(self, plugin):
        ...
        parent_directory = os.path.abspath(os.path.join(os.getcwd(), '..')) #TODO correct path - look at other repo
    ...
    def start(self):
        ...
        while True:
            if self.on_first_layer == True:
                print('loop running ****')
                try:
                    snapshot = capture_jpeg(self.plugin) #TODO replace argument with nozzle cam config
                    self.save_snapshot_as_jpeg(snapshot)
